...
class TwinstarArmoryApi {
    ...
    public static function InitCharactersFromBoomTime(SQL $sql): void {
        $httpRequest = new XMLHTTPRequest("http://armory.twinstar.cz/guild-info.xml?r=Kronos+III&gn=Boom+Time");
    ...
    public static function InitCharacterByXML(SimpleXMLElement $xml, SQL $sql) {
        ...
        $query = "INSERT INTO characters (charName, level, classId, raceId, genderId, lastModified, highestPvpRank) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE level=VALUES(level), classId=VALUES(classId), raceId=VALUES(raceId), genderId=VALUES(genderId), lastModified=VALUES(lastModified), highestPvpRank=VALUES(highestPvpRank)";
        ...
        foreach ($xml->{"characterInfo"}->{"characterTab"}->{"items"}->children() as $child) {
            ...
            $query = "INSERT INTO items (charName, itemName, slot, icon, rarity, level, lastSeen, firstSeen) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE slot=VALUES(slot), icon=VALUES(icon), rarity=VALUES(rarity), level=VALUES(level)";
            $sql->execute($query, [$charName, $itemName, $slot, $icon, $child["rarity"], $child["level"], $lastModified, $lastModified]);
